...
#     def __init__(self, d_model, nhead, num_encoder_layers, dim_feedforward, dropout, output_size):
#         super(Context_Transformer, self).__init__()
#         encoder_layer = nn.TransformerEncoderLayer(
#             d_model=d_model, 
#             nhead=nhead, 
...
#     def __init__(self, x_dim=None, c_dim=None, context_model=None, trainable_q0=False, num_flows=5, hidden_features=128, num_blocks=2, dropout_probability=0.0, use_residual_blocks=True, use_batch_norm=False, lr=0.001, temperature=1.0, weight_decay=1.0, p=None):
#         super(Conditional_MAF, self).__init__()
#         assert context_model is not None, "context_model must be provided"
#         assert isinstance(context_model, (Context_Model, nn.Module)), "context_model must be a instance of Context_Model or nn.Module"
...
#     def fit(self, X_train, c_train, num_epochs, relative_improvement_threshold=1e-2, early_stopping_patience=5, verbose=True):
#         self.train()
#         best_loss = float("inf")
#         patience_counter = 0
...
#     def generate_sample_paths(self, num_sample_paths, num_samples, context, noise=0.0) -> np.ndarray:
#         self.eval()
#         sample_paths = []
#         with torch.no_grad():
#             for _ in range(num_sample_paths):
...
#             }
#         else:
#             raise ValueError(f"Unbekannte Klasse für Context-Modell: {self.context_model.__class__.__name__}")
